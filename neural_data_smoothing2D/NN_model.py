...
class CustomLoss(nn.Module):
	...
	def forward(self, outputs, labels):
		...
		loss = 0.5 * torch.sum(diff*diff, axis=1) * self.tensor_constants.ds # self.tensor_constants.EI * 
		return loss.mean()
...
class CurvatureSmoothing2DLoss(nn.Module):
	...
	def potential_energy(self):
		V = 0.5 * torch.sum(self.tensor_constants.EI * self.kappa_hat*self.kappa_hat, axis=1) * self.tensor_constants.ds
		return V.mean()
		
	def derivative_penalty(self, outputs):
		kappa_hat_s_scaled = torch.einsum('nj,ij->ni', outputs[..., 1:], self.tensor_constants.pca_components) ## n is batch size
		kappa_hat_s = kappa_hat_s_scaled * self.tensor_constants.pca_std + self.tensor_constants.pca_mean
	...
	def data_matching_cost(self, inputs, outputs):
		...
		position_difference = inputs- coeff2position_torch(outputs, self.tensor_constants)
		Phi = 0.5 * self.tensor_constants.chi_r * torch.sum(position_difference*position_difference, axis=1)
		return Phi.mean()
...
class CurvatureSmoothing2DNet(nn.Module):
	def __init__(self, input_size, output_size):
		...
		self.activation = nn.SiLU() # nn.ReLU() # nn.GELU() # convergence speed: GELU > SiLU > ReLU
...
class CurvatureSmoothing2DModel:
	def __init__(self, tensor_constants, input_data, num_epochs=1, batch_size=128, eval_batch_size=100, labels=None):
		self.tensor_constants = tensor_constants
		...
		self.loss_fn = CurvatureSmoothing2DLoss(tensor_constants) # CustomLoss(tensor_constants) # 
	...
	def train_valid_test_split(self):
		...
		training_set, validation_set, test_set = random_split(self.input_data, [0.8, 0.1, 0.1], generator=generator)
		self.training_loader = DataLoader(training_set, batch_size=self.batch_size, shuffle=True)
		self.validation_loader = DataLoader(validation_set, batch_size=self.eval_batch_size, shuffle=True)
		self.test_loader = DataLoader(test_set, batch_size=len(test_set))
		self.num_train_interation = len(self.training_loader)
		print('# training samples:', len(training_set), '# validation samples:', len(validation_set), '# test samples:', len(test_set))
	...
	def train_one_epoch(self, epoch_idx, steps_to_check=100):
		...
		for i, inputs in enumerate(self.training_loader): # (inputs, labels)
			...
			if i % steps_to_check == steps_to_check-1 or i == self.num_train_interation - 1:
				last_loss = running_loss / (i % steps_to_check + 1)
				print(f"Epoch {epoch_idx+1}/{self.num_epochs}, Step [{i+1}/{self.num_train_interation}], Loss: {last_loss:.8f}")
				running_loss = 0.
	...
	def model_train(self):
		for epoch_idx in range(self.num_epochs):
			...
			print(f'Losses: train {self.train_losses[-1]:.8f} valid {avg_vloss:.8f}')
...
def main():
	...
	training_data = np.load(folder_name + 'training_data_set1.npy', allow_pickle='TRUE').item() # training_data_set1
	...
	model = CurvatureSmoothing2DModel(tensor_constants, input_data, num_epochs, batch_size=batch_size, labels=true_kappa)
