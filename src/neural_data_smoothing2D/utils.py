...
def curvature2position(curvature, n_elem=100, ds=0.002, L=0.2):
    ...
    position[..., 1:] = (
        np.cumsum(np.stack([np.cos(theta), np.sin(theta)], axis=1), axis=-1)
        * ds
    )
...
def coeff2curvature_torch(coeff, tensor_constants):
    ...
    kappa_hat_scaled = torch.einsum(
        "nj,ij->ni", coeff, tensor_constants.pca_components
    )  ## n is batch size
    kappa_hat = (
        kappa_hat_scaled * tensor_constants.pca_std + tensor_constants.pca_mean
    )
    return kappa_hat
...
def curvature2position_torch(curvature, tensor_constants):
    ...
    r_j = torch.hstack(
        (
            x_hat[:, tensor_constants.idx_data_pts],
            y_hat[:, tensor_constants.idx_data_pts],
        )
    )
    return r_j
