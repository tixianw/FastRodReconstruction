...
def pos_dir_to_input(pos: np.ndarray, dir: np.ndarray) -> np.ndarray:
	...
	inputs : np.ndarray = np.hstack([pos, dir[:,:,0,:], dir[:,:,-1,:]]) # only take d1 and d3 for dir
	return inputs
...
def coeff2strain(coeff, pca): # , nominal_shear=np.vstack([np.zeros([2,100]), np.ones(100)])):
	num_strain = len(pca)
	strain = []
	start_coeff_idx = 0
	for i in range(num_strain):
		end_coeff_idx = start_coeff_idx + pca[i].n_components
		strain.append(pca[i].approximate(coeff[:,start_coeff_idx:end_coeff_idx]))
...
def coeff2strain_torch(coeff, tensor_constants):
	...
	for i in range(num_strain):
		...
		kappa_hat_scaled = torch.einsum('nj,ij->ni', coeff[:,start_coeff_idx:end_coeff_idx], tensor_constants.pca_components[:,start_coeff_idx:end_coeff_idx]) ## n is batch size
		kappa_hat = kappa_hat_scaled * tensor_constants.pca_std[i,:] + tensor_constants.pca_mean[i,:]
		strain.append(kappa_hat)
		start_coeff_idx = end_coeff_idx
	return torch.stack(strain, axis=1)

def strain2posdir_torch(strain, tensor_constants):
	pos_dir = forward_path_torch(tensor_constants.dl, tensor_constants.nominal_shear, strain)
	return [pos_dir[0][..., tensor_constants.idx_data_pts], pos_dir[1][..., tensor_constants.idx_data_pts]]
...
def forward_path_torch(dl, shear, kappa):
	directors = [torch.stack([torch.diag(torch.Tensor([1,-1,-1])) for i in range(len(kappa))], axis=0)]
	Rotation = get_rotation_matrix_torch(kappa * _aver(dl))
	for i in range(dl.shape[0] - 1):
		directors.append(torch.einsum('nij,njk->nik', directors[-1], Rotation[...,i]))
...
def get_rotation_matrix_torch(axis):
	...
	K2 = torch.einsum('nik,njk->nijk', axis, axis) - torch.eye(3)[None,...,None]

	Rotation = K * torch.sin(angle)[:,None,None,:] + K2 * (1 - torch.cos(angle))[:,None,None,:] + torch.eye(3)[None,...,None]
