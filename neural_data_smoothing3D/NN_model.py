...
class CustomLoss(nn.Module):
	...
	def forward(self, outputs, labels):
		...
		loss = 0.5 * torch.sum(diff*diff * _aver(self.tensor_constants.dl), axis=(1,2)) # *self.tensor_constants.EI
		return loss.mean()
...
class CurvatureSmoothing3DLoss(nn.Module):
	...
	def potential_energy(self):
		V = 0.5 * torch.sum(torch.einsum('nik,ijk,njk->nk', self.kappa_hat, self.tensor_constants.bend_twist_stiff, self.kappa_hat) * _aver(self.tensor_constants.dl), axis=1)
		return V.mean()

	def data_matching_cost(self, inputs, outputs):
		input_pos = inputs[:,:3,:]
		input_dir = torch.stack([inputs[:,3:6,:], torch.cross(inputs[:,6:9,:], inputs[:,3:6,:], axis=1), inputs[:,6:9,:]], axis=2)
		...
		Phi = 0.5 * self.tensor_constants.chi_r * torch.sum(pos_difference*pos_difference, axis=1) + \
			0.5 * self.tensor_constants.chi_d * torch.sum(dir_difference*dir_difference, axis=1)
		return Phi.mean()
...
class CurvatureSmoothing3DNet(nn.Module):
	def __init__(self, input_size, output_size):
		super(CurvatureSmoothing3DNet, self).__init__()
		self.input_size = input_size
		self.output_size = output_size
...
class CurvatureSmoothing3DModel:
	def __init__(self, tensor_constants, input_data, num_epochs=1, batch_size=128, eval_batch_size=100, labels=None):
		...
		self.loss_fn = CurvatureSmoothing3DLoss(tensor_constants) # CustomLoss(tensor_constants) # 
...
def main():
	...
	training_data = np.load(folder_name + 'training_data_set.npy', allow_pickle='TRUE').item() # training_data_set1
