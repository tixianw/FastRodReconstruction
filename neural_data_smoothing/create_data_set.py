...
class PCA:
    ...
    def reduce(self, X):
        X_hat = self.transform(X) @ self.components.T + self.mean
        return X_hat
    
    def approximate(self, coeff):
        X_hat = coeff @ self.components.T + self.mean
        return X_hat
...
def main():
    ...
    # np.save('Data/training_data_set.npy', data)
    ...
    for ii in range(len(idx_list)):
        ...
        # plt.plot(np.linspace(0,1,input_data.shape[-1]), input_data[i,:])
        # plt.plot(np.linspace(0,1,true_pos.shape[-1]), true_pos[i,:])
