...
from utils import coeff2strain, coeff2posdir, pos_dir_to_input
...
def main():
	...
	idx_list = np.random.randint(n_training_data, size=10) # [i*250 for i in range(10)]
	fig = plt.figure(2)
	ax = fig.add_subplot(111, projection='3d')
	fig2, axes = plt.subplots(ncols=3, sharex=True, figsize=(16, 5))
	for ii in range(len(idx_list)):
		i = idx_list[ii]
		ax.plot(posdir_rand[0][i,0,:], posdir_rand[0][i,1,:], posdir_rand[0][i,2,:], ls='-', color=color[ii])
